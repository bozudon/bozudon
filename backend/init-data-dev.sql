BEGIN TRANSACTION;

INSERT INTO account (username, display_name, note, uri, inbox_url, outbox_url, shared_inbox_url, followers_url, private_key, public_key)
VALUES
    (
        'user1',
        'User 1',
        'Hi! I am User 1',
        'https://localhost:8080/users/user1',
        'https://localhost:8080/users/user1/inbox',
        'https://localhost:8080/users/user1/outbox',
        'https://localhost:8080/inbox',
        'https://localhost:8080/users/user1/followers',
        '-----BEGIN PRIVATE KEY-----

i5o+2GJTJHWRfhSFW91fbXHG
-----END PRIVATE KEY-----
',
        '-----BEGIN PUBLIC KEY-----

jwIDAQAB
-----END PUBLIC KEY-----
'
    ),
    (
        'user2',
        'user 2',
        '',
        'https://localhost:8080/users/user2',
        'https://localhost:8080/users/user2/inbox',
        'https://localhost:8080/users/user2/outbox',
        'https://localhost:8080/inbox',
        'https://localhost:8080/users/user2/followers',
        '-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
',
        '-----BEGIN PUBLIC KEY-----

kwIDAQAB
-----END PUBLIC KEY-----
'
    ),
    (
        'user3',
        'User 3',
        '',
        'https://localhost:8080/users/user3',
        'https://localhost:8080/users/user3/inbox',
        'https://localhost:8080/users/user3/outbox',
        'https://localhost:8080/inbox',
        'https://localhost:8080/users/user3/followers',
        '-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
',
        '-----BEGIN PUBLIC KEY-----

lQIDAQAB
-----END PUBLIC KEY-----
'
    );

INSERT INTO user_ (email, encrypted_password, account_id)
VALUES
    (
        'user1@example.com',
        '$2y$10$5nGAZW6/iPChMaraRQ3Wqu7.T3qUctRvtZZ.FfcBF/ot5wi9W2f9q', -- "password"
        1
    ),
    (
        'user2@example.com',
        '$2y$10$9D4m7pZy74MEiXRLUeMdLuSETDS0ri.xUnhcLYbeGBAzQUXmfPfaO', -- "password"
        2
    ),
    (
        'user3@example.com',
        '$2y$10$KZ327gRc0FUON5BZAVL3pe5xIIUzm/p8iY9Rd92B8NAHYIXzfNkwm', -- "password"
        3
    );

INSERT INTO status (created_at, updated_at, text, in_reply_to_id, reblog_of_id, account_id, uri, media_ids)
VALUES
    ('2023-07-11 01:19:03', '2023-07-11 01:19:03', '初めての投稿', NULL, NULL, 1, 'https://localhost:8080/users/user1/statuses/1', '{}'),
    ('2023-07-11 01:23:18', '2023-07-11 01:23:18', '', NULL, 1, 2, 'https://localhost:8080/users/user1/statuses/2', '{}'),
    ('2023-07-11 01:23:25', '2023-07-11 01:23:25', '動いてるじゃん、すげー', NULL, NULL, 2, 'https://localhost:8080/users/user1/statuses/3', '{}'),
    ('2023-07-11 07:58:10', '2023-07-11 07:58:10', '', NULL, 1, 3, 'https://localhost:8080/users/user1/statuses/4', '{}'),
    ('2023-07-11 07:59:07', '2023-07-11 07:59:07', 'Hello, world!', NULL, NULL, 3, 'https://localhost:8080/users/user1/statuses/5', '{}'),
    ('2023-07-11 08:15:20', '2023-07-11 08:15:20', 'おはよう', 5, NULL, 2, 'https://localhost:8080/users/user1/statuses/6', '{}'),
    ('2023-07-11 09:20:58', '2023-07-11 09:20:58', '眠い', NULL, NULL, 2, 'https://localhost:8080/users/user1/statuses/7', '{}'),
    ('2023-07-12 13:56:24', '2023-07-12 13:56:24', 'おやすみ', 6, NULL, 1, 'https://localhost:8080/users/user1/statuses/8', '{}'),
    ('2023-07-12 14:02:25', '2023-07-12 14:02:25', 'ええ…', 8, NULL, 3, 'https://localhost:8080/users/user1/statuses/9', '{}');

INSERT INTO follow (account_id, target_account_id)
VALUES
    (1, 2),
    (2, 3),
    (3, 1),
    (1, 3);

INSERT INTO favorite (account_id, status_id)
VALUES
    (2, 1),
    (3, 1),
    (1, 3),
    (1, 7),
    (3, 7),
    (1, 5),
    (2, 5);

INSERT INTO app (name, redirect_uri, scopes, website, client_id, client_secret)
VALUES
    ('Sample App', 'http://127.0.0.1:3000/auth/callback', 'read write follow push', 'https://example.com', '0CY6txjKvHC9Y1uWpmbl56nnHbY7skhJrDOUb-kps2U', 'bds_iQFzFHkHRt_GPJt4GkngyKo5oMvmxAhp2Bs6bcfh-yc');

COMMIT;
